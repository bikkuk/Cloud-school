...
class ProjectStore:
    _INVALID_PROJECT_NAME = re.compile(r"[\\/]")

    def __init__(self, root: str = "studio_projects"):
        self.root = Path(root)
        self.root.mkdir(parents=True, exist_ok=True)

    def _validated_project_path(self, project_name: str) -> Path:
        name = (project_name or "").strip()
        if not name:
            raise ValueError("Project name cannot be empty.")
        if self._INVALID_PROJECT_NAME.search(name) or name in {".", ".."} or ".." in name:
    ...
    def project_path(self, project_name: str) -> Path:
        return self._validated_project_path(project_name)
