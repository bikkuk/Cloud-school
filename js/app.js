(() => {
  ...
  const accountState = {
    ...
    accounts: safeJsonParse('nk_accounts', [])
  };
  ...
  const resetGuestPoints = () => {
    localStorage.setItem('nk_guest_points', '0');
    localStorage.setItem('nk_guest_level', '1');
  };
  ensureGuestData();
  ...
  const injectAccountModal = () => {
    ...
    wrap.innerHTML = `
      <div class="modal-panel">
        ...
        <p class="muted">Konto-Daten werden lokal in diesem Browser (localStorage) gespeichert. Keine externe Cloud-Speicherung für diese Kontofunktion.</p>
        <div class="card-grid">
