...
ANCHOR_PROMPT = """You are an AI video anchor for seniors, running fully local.
...
def is_unclear(text: str) -> bool:
    ...
    if len(plain.split()) < 4:
        return True
    return plain in {"help", "explain", "question", "what about", "tell me"}
...
def call_ollama(user_prompt: str, system_prompt: str) -> str:
    payload = {
        "model": MODEL_NAME,
...
def answer_question(user_input: str) -> str:
    ...
    if blocked:
        return blocked
    try:
        return call_ollama(user_input, SYSTEM_PROMPT)
    except requests.RequestException:
        return "Local model unavailable. Please start Ollama and confirm qwen2.5:7b is installed."


def make_anchor_script(topic: str) -> str:
    blocked = blocked_or_none(topic)
    if blocked:
        return blocked
    try:
        return call_ollama(topic, ANCHOR_PROMPT)
    except requests.RequestException:
